...
def register_handlers(bot: telebot.TeleBot, config: BotConfig):
    @bot.message_handler(commands=['start'])
    def start(message: Message):
        bot.send_message(message.chat.id, config.greeting_text)

    @bot.message_handler(func=lambda m: m.from_user.id != config.admin_id, content_types=['text'])
    def handle_client_message(message: Message):
        ...
        bot.send_message(
            ...
            f"Ник: @{message.from_user.username or 'нет ника'}\n"
            f"ID: {user_id}\n\n"
            f"{text}"
        )
...
def _run_bots(bots):
    ...
    for current_bot in bots:
        thread = threading.Thread(
            target=current_bot.infinity_polling,
